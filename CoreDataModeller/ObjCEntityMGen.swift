...
class ObjCEntityMGen: NSObject {
    ...
    func setupArrays() {
        comment = ["//","//  \(entityName).m","//  \(title)","//","//  Created by \(username) on \(date).","//  Copyright (c) 2015 \(username). All rights reserved.","//","","#import <CoreData/CoreData.h>","","@interface \(entityName) : NSManagedObject",""];
